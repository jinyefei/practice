...
<body >
   ...
   <div id="page">
   	...
   	<div id="logoleft">
		...
		<form action="${pageContext.request.contextPath}/payAndCreateOrderDetailsServlet?map=mapnew&uno=${user.username }&uid=${user.uid}" method="post">
		...
		<% Map<Product,Integer> mapnew=(Map)session.getAttribute("mapnew");
  		if(mapnew==null){
   			  out.println("温馨提示：您购物车为空!!!");
    	 return ;
 		 }
 		 Set<Product> key=mapnew.keySet(); %>
  		
 		<c:forEach items="${mapnew.keySet()}" var="product">
			<tr>
			<td ><img style="width:50px;height:50px;text-align:center" src="${pageContext.request.contextPath}/images/${product.pimage }"/></td>
			<td>${product.pname }</td><td>${product.market_price }</td><td>${product.shop_price }</td><td>${mapnew.get(product)}</td><td>${product.shop_price }</td>
			</tr> 
		</c:forEach>
        <% 
  	//  计算总金额 sums
  		...
  		sums=sums+product.getShop_price()*mapnew.get(product);
        }
		...
		<tr>
		<td colspan="2"><input type="hidden" name="uno" value="${user.uid }" /></td>
		
		<td colspan="4" style="text-align:right;padding-right:15px">
				<div id="buy_can">
				<a id="can" href="${pageContext.request.contextPath}/canceOrderServlet?map=mapnew" >
					<font >取消订单</font></a>
						&nbsp;&nbsp;&nbsp;&nbsp;
